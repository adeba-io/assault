...
namespace Assault.Editors
{
    ...
    public class FighterControllerEditor : Editor
    {
        ...
        ReorderableList _inputComboNodesRL, _standingTechniquesRL;
        ...
        readonly GUIContent gui_inputComboNodes = new GUIContent("Input Combo Nodes");
        ...
        {
            _standingTechniques = new ReorderableList(serializedObject, serializedObject.FindProperty("_standing"), false, true, true, true)
            {
                elementHeight = 70,
                drawHeaderCallback = (Rect rect) =>
                {
                    ...
                    rect.width *= 0.95f;
                    EditorGUI.LabelField(rect, gui_standingTechniques, guis_rlHeader);
                },
                drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                {
                    var element = _standingTechniques.serializedProperty.GetArrayElementAtIndex(index);
                    ...
                    catch
                    {
                        ...
                        Rect createTech = new Rect(rect_technique.x, noTech.yMax, rect_technique.width * 0.49f, rect_technique.height - noTech.height);
                        ...
                        {
                            AddHandler(new AddData { createNew = true, increment = false, index = index, list = RLists.StndTechniques });
                        }
                        if (GUI.Button(addTech, gui_assignTechnique))
                        {
                            GenericMenu menu = new GenericMenu();

                            var guids = AssetDatabase.FindAssets("", new[] { FighterLibrary.GetMyGroundedTechniqueDirectory(_target.name) });
                            for (int i = 0; i < guids.Length; i++)
                            {
                                var path = AssetDatabase.GUIDToAssetPath(guids[i]);
                                menu.AddItem(new GUIContent(Path.GetFileNameWithoutExtension(path)),
                                    false, AddHandler,
                                    new AddData { createNew = false, increment = false, index = index, path = path, list = RLists.StndTechniques });
                            }
                ...
                onAddDropdownCallback = (Rect buttonRect, ReorderableList list) =>
                {
                    ...
                    menu.AddItem(new GUIContent("Create new"), false, AddHandler, new AddData { createNew = true, increment = true, list = RLists.StndTechniques });
                    menu.AddItem(new GUIContent("Create empty"), false, AddHandler, new AddData { createNew = false, increment = true, list = RLists.StndTechniques });
                ...
                onRemoveCallback = (ReorderableList list) =>
                {
                    ...
                    {
                        AssetDatabase.DeleteAsset(FighterLibrary.GetMyGroundedTechniqueDirectory(_target.name) + "/" + delTech.name + ASSETEXTENSION);
                    }
                },
            ...
            switch (data.list)
            {
                case RLists.StndTechniques:
                    ...
                    if (data.createNew)
                    {
                        ...
                        string location = FighterLibrary.GetMyGroundedTechniqueDirectory(_target.name) + "/" + newAsset.name + ASSETEXTENSION;
